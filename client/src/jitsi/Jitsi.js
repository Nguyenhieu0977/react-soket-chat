...
const Jitsi = () => {
  ...
  console.log(jitsiStore.jitsiStrore)
  // console.log(jitsiAudio.jitsiStrore._numberOfParticipants)
  ...
  const initialiseJitsi = async () => {
    ...
    const _jitsi = new window.JitsiMeetExternalAPI("localhost:8443", {
      ...
      interfaceConfigOverwrite: {
        ...
        // TOOLBAR_BUTTONS: ['chat', 'camera', 'micro']
      },
    });
    setJitsi(_jitsi)
    ...
    // _jitsi.executeCommand('muteEveryone');
    // _jitsi.executeCommand('toggleChat');
    // _jitsi.executeCommand('toggleCameraMirror');
    ...
    const numberOfParticipants = _jitsi.getNumberOfParticipants();
    console.log(toggleAudioClick)
    // _jitsi.executeCommand('overwriteConfig',
    //   {
    //     toolbarButtons: ['chat', 'micro', 'camera']
    //   }
    // );
    // dispatch(setJitsiStore(_jitsi));
  };
  ...
  //   return () => jitsi?.executeCommand('toggleVideo')?.();
  // }


  const { toggle, fullScreen } = useFullScreen();
  // const toggleFullClick = () => {
  //   toggle();
  // }
  ...
  // useEffect(() => {
  //   if (jitsiStore.videoAllJitsi) toggleVideoClick();

  // }, [jitsiStore.videoAllJitsi]);

  // useEffect(() => {
  //   if(jitsiStore.fullAllJitsi) toggleFullClick();

  // }, [jitsiStore.fullAllJitsi]);
